"""
Grid Experiment #015: LPL Baseline Run (Conv-MLP Hybrid, 10K steps)
...
followed by a linear LPL layer trained with Full LPL rules, extended to 10,000 steps.
"""
...
def main():
    ...
    print("="*70)
    print("GRID EXPERIMENT #015".center(70))
    print("="*70)
    ...
    output_dir = output_base / f"run_015_{EXPERIMENT_CONFIG['dataset']}_{EXPERIMENT_CONFIG['steps']}steps_{EXPERIMENT_CONFIG['architecture']}_{EXPERIMENT_CONFIG['activation']}_{EXPERIMENT_CONFIG['rule']}"
    ...
    for step in range(1, EXPERIMENT_CONFIG['steps'] + 1):
        ...
        # Clear GPU cache very frequently to prevent memory buildup (critical for longer runs)
        if step % 100 == 0:
