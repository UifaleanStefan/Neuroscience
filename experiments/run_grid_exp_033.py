"""
Grid Experiment #033: MNIST with Conv-MLP Hybrid (20k steps)
...
class ConvMLPHybrid2Layer:
    ...
    def forward(self, x: torch.Tensor, return_conv_features=False):
        ...
        # MLP forward (includes ReLU activation in LPL layers)
...
def main():
    ...
    output_dir = output_base / "run_033_mnist_20000steps_conv_mlp_tanh_full_lpl"
    ...
    # Log interval to reduce memory usage (log every 500 steps for long training)
    log_interval = 500
    ...
    try:
        for step in range(1, EXPERIMENT_CONFIG['steps'] + 1):
            ...
            # Export intermediate activations at checkpoints
            if step == 10000:
                print(f"\nExporting activations at {step} steps (checkpoint)...")
                activations_10k = export_activations(model, export_dataset, num_samples=1000)
                
                # Safety check: no NaN in activations
                assert not torch.isnan(activations_10k['conv_features']).any(), \
                    f"ERROR: NaN detected in conv features at step {step}!"
                assert not torch.isnan(activations_10k['mlp_layer1_activations']).any(), \
                    f"ERROR: NaN detected in MLP layer1 activations at step {step}!"
                assert not torch.isnan(activations_10k['mlp_layer2_activations']).any(), \
                    f"ERROR: NaN detected in MLP layer2 activations at step {step}!"
                
                # Safety check: activation std > 0.1 (variance check)
                conv_std_10k = activations_10k['conv_features'].std().item()
                mlp1_std_10k = activations_10k['mlp_layer1_activations'].std().item()
                mlp2_std_10k = activations_10k['mlp_layer2_activations'].std().item()
                print(f"Conv features std at {step} steps: {conv_std_10k:.6f}")
                print(f"MLP Layer1 activation std at {step} steps: {mlp1_std_10k:.6f}")
                print(f"MLP Layer2 activation std at {step} steps: {mlp2_std_10k:.6f}")
                
                torch.save(activations_10k, output_dir / 'activations_10000steps.pt')
