"""
Grid Experiment #073: STL-10 with 1-layer MLP (50k steps)
...
def main():
    ...
    print("="*70)
    print("GRID EXPERIMENT #073".center(70))
    print("="*70)
    ...
    output_dir = output_base / f"run_073_{EXPERIMENT_CONFIG['dataset']}_{EXPERIMENT_CONFIG['steps']}steps_{EXPERIMENT_CONFIG['architecture']}_{EXPERIMENT_CONFIG['activation']}_{EXPERIMENT_CONFIG['rule']}"
    ...
    # Safety check: activation std > 0.1 (non-collapsed)
    ...
    if activation_std_after < 0.1:
        print(f"ERROR: Activation std ({activation_std_after:.6f}) is below 0.1 threshold - REPRESENTATION COLLAPSED!")
        assert False, "Representation collapsed - activation std < 0.1"
    else:
        print(f"OK: Activation std ({activation_std_after:.6f}) is above 0.1 threshold - representation is healthy")
