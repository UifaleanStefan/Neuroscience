"""
Grid Experiment #063: CIFAR-10 with 3-layer MLP (10k steps)

Configuration:
- Dataset: CIFAR-10 (RGB, 32x32 images, flattened to 3072)
- Training steps: 10,000
- Architecture: 3-layer MLP (3072 → 256 → 128 → 64 units)
...
def export_activations(model, dataset, num_samples=1000):
    ...
    for i in range(min(num_samples, len(dataset))):
        ...
        # Flatten image to 1D tensor (32x32 = 3072)
        x = image.flatten()
...
def main():
    ...
    print("="*70)
    print("GRID EXPERIMENT #063".center(70))
    print("="*70)
    ...
    output_dir = output_base / f"run_063_{EXPERIMENT_CONFIG['dataset']}_{EXPERIMENT_CONFIG['steps']}steps_{EXPERIMENT_CONFIG['architecture']}_{EXPERIMENT_CONFIG['activation']}_{EXPERIMENT_CONFIG['rule']}"
    ...
    # Create model (3-layer MLP: 3072 → 128 → 64 → 32)
    ...
    for step in range(1, EXPERIMENT_CONFIG['steps'] + 1):
        ...
        # Flatten images to 1D tensors (32x32 = 3072)
