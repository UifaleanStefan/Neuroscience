"""
Grid Experiment #007: LPL Baseline Run (2-Layer MLP, Extended Training)

Configuration:
- Dataset: Synthetic Shapes
- Training steps: 10,000 (extended from Run #6)
- Architecture: 2-layer MLP (128 units -> 64 units)
- Activation: tanh
- Learning rule: full LPL (Hebbian + Predictive + Stabilization enabled)
- Baseline: none (LPL only)
- Seed: 42 (matches Run #5 and #6)

This run is identical to Run #6 except for training length (10000 vs 5000 steps).
"""
...
def export_activations(model, dataset, num_samples=1000):
    ...
    For 2-layer model, exports final layer (layer 2) activations.
    ...
    for i in range(min(num_samples, len(dataset))):
        ...
        # Forward pass to get activations (returns layer 2 output by default)
        with torch.no_grad():
            y = model.forward(x)  # Returns layer 2 activations
...
def main():
    ...
    EXPERIMENT_CONFIG = {
        ...
        'steps': 10000,  # Changed to 10000 steps
    ...
    print("="*70)
    print("GRID EXPERIMENT #007".center(70))
    print("="*70)
    ...
    output_dir = output_base / f"run_007_{EXPERIMENT_CONFIG['dataset']}_{EXPERIMENT_CONFIG['steps']}steps_{EXPERIMENT_CONFIG['architecture']}_{EXPERIMENT_CONFIG['activation']}_{EXPERIMENT_CONFIG['rule']}"
    ...
    # Create model (2-layer MLP: 1024 -> 128 -> 64)
    ...
    for step in range(1, EXPERIMENT_CONFIG['steps'] + 1):
        ...
        # Use layer 2 (final layer) for consistency with single-layer runs
        weight_norm = torch.norm(model.layer2.W).item()
        y_sample = model.forward(x_t_flat)  # Returns layer 2 output
        ...
        # Safety check: no NaN in weights (check both layers)
        ...
        # Print progress every 1000 steps (less frequent for longer run)
        if step % 1000 == 0:
    ...
    final_weight_norm = torch.norm(model.layer2.W).item()
    print(f"Weight norm (Layer2): {final_weight_norm:.6f}")
    print("="*70)
