"""
Grid Experiment #016: LPL Baseline Run (Conv-MLP Hybrid, 20K steps)
...
followed by a linear LPL layer trained with Full LPL rules, extended to 20,000 steps.
"""
...
def main():
    ...
    print("="*70)
    print("GRID EXPERIMENT #016".center(70))
    print("="*70)
    ...
    output_dir = output_base / f"run_016_{EXPERIMENT_CONFIG['dataset']}_{EXPERIMENT_CONFIG['steps']}steps_{EXPERIMENT_CONFIG['architecture']}_{EXPERIMENT_CONFIG['activation']}_{EXPERIMENT_CONFIG['rule']}"
    ...
    # Log every N steps to reduce memory usage (less frequent for longer runs)
    log_interval = 50  # Log every 50 steps instead of every step
    training_logs = {
        'step': [],
        'weight_norm': [],
        'activation_norm': []
    ...
    # Training loop with batching to reduce memory pressure
    batch_size = 10  # Process 10 samples before applying weight update
    print(f"\nTraining LPL for {EXPERIMENT_CONFIG['steps']} steps (batch size: {batch_size})...")
    ...
    for step in range(1, EXPERIMENT_CONFIG['steps'] + 1):
        ...
        # Clear input tensors after use
        del x_t, x_t1
        
        # Clear GPU cache every batch_size steps to reduce memory buildup
        if step % batch_size == 0:
            torch.cuda.empty_cache()
            gc.collect()
        
        # Safety check: no NaN in weights (check linear layer) - check periodically
        if step % 100 == 0:
        ...
        # Print progress every 2000 steps (adjusted for 20K steps)
        if step % 2000 == 0:
