"""
Grid Experiment #069: CIFAR-10 Non-LPL Baseline (Standard Backpropagation)
...
instead of local learning rules (LPL) for CIFAR-10 dataset.
"""
...
class StandardMLP(nn.Module):
    ...
    Architecture: Input (3072) -> Linear (128) -> tanh (scaled)
    ...
    def __init__(self, d_in: int, d_out: int):
        ...
        Args:
            d_in: Input dimension (32 * 32 = 3072 for flattened CIFAR-10 images)
...
def export_embeddings(model, dataset, num_samples=1000, device='cpu'):
    ...
    with torch.no_grad():
        for i in range(min(num_samples, len(dataset))):
            ...
            # Flatten image to 1D (32x32 = 3072)
            if image.dim() > 1:
                image = image.flatten()
...
def main():
    ...
    EXPERIMENT_CONFIG = {
        ...
        'seed': 42,
        'd_in': 3 * 32 * 32,           # 3072 for flattened 3x32x32 images
        'd_out': 128,               # 128 units
    ...
    print("="*70)
    print("GRID EXPERIMENT #069".center(70))
    print("="*70)
    ...
    print(f"  Input dim: {EXPERIMENT_CONFIG['d_in']} (3x32x32 flattened)")
    ...
    output_dir = output_base / f"run_069_{EXPERIMENT_CONFIG['dataset']}_{EXPERIMENT_CONFIG['steps']}steps_{EXPERIMENT_CONFIG['architecture']}_{EXPERIMENT_CONFIG['activation']}_{EXPERIMENT_CONFIG['rule']}"
    ...
    for step in range(1, EXPERIMENT_CONFIG['steps'] + 1):
        ...
        # Flatten images to 1D (3x32x32 = 3072)
