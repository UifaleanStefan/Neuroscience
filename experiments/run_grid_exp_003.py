"""
Grid Experiment #003: LPL Baseline Run (Extended Training)

Configuration:
- Dataset: Synthetic Shapes
- Training steps: 10,000 (extended from Run #2)
- Architecture: 1-layer MLP (128 units)
- Activation: tanh
- Learning rule: full LPL (Hebbian + Predictive + Stabilization enabled)
- Baseline: none (LPL only)
- Seed: 42 (matches Run #1 and #2)

This run is identical to Run #1 and #2 except for training length (10000 vs 1000/5000 steps).
"""
...
def main():
    """
    Run grid experiment #003.
    """
    # Fixed random seed for reproducibility (matches Run #1 and #2)
    ...
    print("="*70)
    print("GRID EXPERIMENT #003".center(70))
    print("="*70)
    ...
    output_dir = output_base / f"run_003_{EXPERIMENT_CONFIG['dataset']}_{EXPERIMENT_CONFIG['steps']}steps_{EXPERIMENT_CONFIG['architecture']}_{EXPERIMENT_CONFIG['activation']}_{EXPERIMENT_CONFIG['rule']}"
