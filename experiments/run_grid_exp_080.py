"""
Grid Experiment #080: STL-10 with 3-layer MLP (10k steps)

Configuration:
- Dataset: STL-10 (RGB, 96x96 images, flattened to 27648)
- Training steps: 10,000
- Architecture: 3-layer MLP (27648 → 256 → 128 → 64 units)
...
def export_activations(model, dataset, num_samples=1000):
    ...
    for i in range(min(num_samples, len(dataset))):
        ...
        # Flatten image to 1D tensor (96x96 = 27648)
        x = image.flatten()
...
def main():
    ...
    print("="*70)
    print("GRID EXPERIMENT #080".center(70))
    print("="*70)
    ...
    output_dir = output_base / f"run_080_{EXPERIMENT_CONFIG['dataset']}_{EXPERIMENT_CONFIG['steps']}steps_{EXPERIMENT_CONFIG['architecture']}_{EXPERIMENT_CONFIG['activation']}_{EXPERIMENT_CONFIG['rule']}"
    ...
    # Create model (3-layer MLP: 27648 → 128 → 64 → 32)
    ...
    midpoint_step = EXPERIMENT_CONFIG['steps'] // 2
    ...
    for step in range(1, EXPERIMENT_CONFIG['steps'] + 1):
        ...
        # Flatten images to 1D tensors (96x96 = 27648)
        ...
        activation_std_layer1 = y1_sample.std().item()
        ...
        # Export activations at midpoint (step 5000)
        if step == midpoint_step:
            print(f"\nReached midpoint at step {step}. Exporting activations...")
            activations_midpoint = export_activations(model, export_dataset, num_samples=1000)
            
            # Safety check: no NaN in activations (all three layers)
            assert not torch.isnan(activations_midpoint['layer1_activations']).any(), \
                f"ERROR: NaN detected in layer1 activations at midpoint (step {step})!"
            assert not torch.isnan(activations_midpoint['layer2_activations']).any(), \
                f"ERROR: NaN detected in layer2 activations at midpoint (step {step})!"
            assert not torch.isnan(activations_midpoint['layer3_activations']).any(), \
                f"ERROR: NaN detected in layer3 activations at midpoint (step {step})!"
            
            # Safety check: activation std > 0.1 (all three layers)
            layer1_std_midpoint = activations_midpoint['layer1_activations'].std().item()
            layer2_std_midpoint = activations_midpoint['layer2_activations'].std().item()
            layer3_std_midpoint = activations_midpoint['layer3_activations'].std().item()
            print(f"Layer1 activation std at midpoint: {layer1_std_midpoint:.6f}")
            print(f"Layer2 activation std at midpoint: {layer2_std_midpoint:.6f}")
            print(f"Layer3 activation std at midpoint: {layer3_std_midpoint:.6f}")
            
            if layer1_std_midpoint < 0.1:
                print(f"WARNING: Layer1 activation std ({layer1_std_midpoint:.6f}) is below 0.1 threshold at midpoint!")
            if layer2_std_midpoint < 0.1:
                print(f"WARNING: Layer2 activation std ({layer2_std_midpoint:.6f}) is below 0.1 threshold at midpoint!")
            if layer3_std_midpoint < 0.1:
                print(f"WARNING: Layer3 activation std ({layer3_std_midpoint:.6f}) is below 0.1 threshold at midpoint!")
            
            torch.save(activations_midpoint, output_dir / 'activations_midpoint.pt')
            print(f"Saved midpoint activations to {output_dir / 'activations_midpoint.pt'}")
