"""
Grid Experiment #004: LPL Baseline Run (Extended Training - Long Regime)

Configuration:
- Dataset: Synthetic Shapes
- Training steps: 50,000 (extended from Run #3)
- Architecture: 1-layer MLP (128 units)
- Activation: tanh
- Learning rule: full LPL (Hebbian + Predictive + Stabilization enabled)
- Baseline: none (LPL only)
- Seed: 42 (matches Run #1, #2, and #3)

This run is identical to Runs #1-3 except for training length (50000 vs 1000/5000/10000 steps).
"""
...
def main():
    """
    Run grid experiment #004.
    """
    # Fixed random seed for reproducibility (matches Run #1, #2, and #3)
    ...
    print("="*70)
    print("GRID EXPERIMENT #004".center(70))
    print("="*70)
    ...
    output_dir = output_base / f"run_004_{EXPERIMENT_CONFIG['dataset']}_{EXPERIMENT_CONFIG['steps']}steps_{EXPERIMENT_CONFIG['architecture']}_{EXPERIMENT_CONFIG['activation']}_{EXPERIMENT_CONFIG['rule']}"
    ...
    final_weight_norm = torch.norm(model.W).item()
