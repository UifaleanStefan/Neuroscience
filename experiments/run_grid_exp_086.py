"""
Grid Experiment #086: STL-10 Non-LPL Baseline (Standard Backpropagation)

Configuration:
- Dataset: STL-10 (RGB, 96x96 images, flattened to 27648)
- Training steps: 10,000
- Architecture: 1-layer MLP (27648 → 128 units)
...
instead of local learning rules (LPL) for STL-10 dataset.
"""
...
class StandardMLP(nn.Module):
    ...
    Architecture: Input (27648) -> Linear (128) -> tanh (scaled)
    ...
    def __init__(self, d_in: int, d_out: int):
        ...
        Args:
            d_in: Input dimension (96 * 96 = 27648 for flattened STL-10 images)
...
def export_embeddings(model, dataset, num_samples=1000, device='cpu'):
    ...
    with torch.no_grad():
        for i in range(min(num_samples, len(dataset))):
            ...
            # Flatten image to 1D (96x96 = 27648)
            if image.dim() > 1:
                image = image.flatten()
...
def main():
    ...
    EXPERIMENT_CONFIG = {
        ...
        'seed': 42,
        'd_in': 3 * 96 * 96,           # 27648 for flattened 3x96x96 images
        'd_out': 128,               # 128 units
    ...
    print("="*70)
    print("GRID EXPERIMENT #086".center(70))
    print("="*70)
    ...
    print(f"  Input dim: {EXPERIMENT_CONFIG['d_in']} (3x96x96 flattened)")
    ...
    output_dir = output_base / f"run_086_{EXPERIMENT_CONFIG['dataset']}_{EXPERIMENT_CONFIG['steps']}steps_{EXPERIMENT_CONFIG['architecture']}_{EXPERIMENT_CONFIG['activation']}_{EXPERIMENT_CONFIG['rule']}"
    ...
    for step in range(1, EXPERIMENT_CONFIG['steps'] + 1):
        ...
        # Flatten images to 1D (3x96x96 = 27648)
